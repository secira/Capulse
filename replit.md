...
-   **Subscription Model**: Tiered access with 4 plans. DB enum values unchanged (FREE, TARGET_PLUS, TARGET_PRO, HNI) — display names rebranded to Starter, Growth, Pro, Elite. **14-day full-access trial + one-time 7-day extension**: FREE users get COMPLETE access to all features for 14 days from `created_at` (all AI tools, F&O, Trade Now, 1 broker connection). They may claim ONE 7-day extension via `POST /account/extend-trial` — eligibility tracked by `User.trial_extended_at`/`trial_extended_until` columns. Extension is offered in the sidebar when ≤3 days remain (or after expiry, up to +7 days post-expiry). After the trial (and extension) ends, all requests redirect to `/pricing`. Enforced via `check_trial_expiry` `@before_request` hook in `app.py` (`_TRIAL_EXEMPT_ENDPOINTS` frozenset covers auth, pricing, payments, settings, `extend_trial`). Trial logic lives in `User.is_trial_active()`, `User.has_full_access()`, `User.trial_days_remaining()`, `User.can_extend_trial()`, `User.extend_trial()`, with `TRIAL_BASE_DAYS=14` and `TRIAL_EXTENSION_DAYS=7` constants. `paid_plan_required` decorator and all manual plan checks use `has_full_access()`. Sidebar shows full menu to all users; FREE users see a yellow countdown banner that turns urgent (red border) at ≤4 days. Admins always exempt.
