...
-   **Subscription Model**: Tiered access with 4 plans. DB enum values unchanged (FREE, TARGET_PLUS, TARGET_PRO, HNI) — display names rebranded to Starter, Growth, Pro, Elite. FREE=0 broker connections, GROWTH=1, PRO/ELITE=3. **Central FREE-plan access guard** in `app.py` (`enforce_free_plan_access` `@before_request` hook) uses a whitelist (`_FREE_PLAN_ALLOWED`) — FREE users can ONLY access: Dashboard, Live Market Pulse, Portfolio Analysis, public pages, auth, settings, and payment/upgrade flows. All other endpoints redirect to /pricing. Admins always exempt. Sidebar also hides paid menus for FREE users (upgrade box shown instead).
