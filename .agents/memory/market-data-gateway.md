...
description: Uniform market data fallback chain and AI routing — created as part of Task #39.
---
...
2. **TrueData** — only when `data_api_plan.plan_type == 'nse_truedata'` and credentials set
3. **System Dhan** — any connected `DataApiBroker` (via `dhan_service`)
4. **NSEPython** — `nse_quote()` direct NSE scrape
5. **yfinance** — universal last resort

## Gateway API
- `get_price(symbol, user_id)` → `{value, source, success}`
- `get_ohlcv(symbol, days, user_id)` → `{df, source, success}`
- `get_quotes(symbols, user_id)` → `{quotes: {sym: {price, change_percent, source}}, source, success}`
...
## Where each area was updated
- `services/iscore/data_fetcher.py` — delegates to `gateway.get_ohlcv()`
- `services/nse_realtime_service.py` — `get_nse_indices()` calls `gateway.get_index_prices()` as P0
- `services/nifty_options_engine.py` — `get_market_indices()` calls `gateway.get_index_prices()` as P0
- `routes_daily_signals.py` — `_fetch_nse_nifty50_stocks()` calls `gateway.get_quotes()` as P0

**Why:** Before this change, each area had its own bespoke Dhan → yfinance chain, bypassing the admin broker pool entirely. The gateway ensures the admin's configured broker (whatever it is) always gets priority, and TrueData is properly respected when the admin has configured it.
