...
def get_nse_quote(symbol):
    ...
    try:
        ...
        # ── Equity path — use the same realtime service every other
        # screen on the site uses (Portfolio Hub, Equities refresh,
        # Research, etc.). Priority chain inside the service:
        #   1. User's configured Data API broker (Dhan/Zerodha/Fyers/…)
        #   2. Any connected system Dhan
        #   3. yfinance fast_info (no slow history download)
        uid = current_user.id if getattr(current_user, "is_authenticated", False) else None
        from services.nse_realtime_service import get_stock_quote as rt_get_stock_quote
        rt = rt_get_stock_quote(sym, user_id=uid)
        if rt and rt.get('success') and float(rt.get('current_price') or 0) > 0:
            price = float(rt['current_price'])
            return jsonify({'success': True, 'data': {
                'symbol': sym,
                'company_name': sym,
                'current_price': price,
                'previous_close': price - float(rt.get('change') or 0),
                'change_amount': float(rt.get('change') or 0),
                'change_percent': float(rt.get('change_percent') or 0),
                ...
                'market_cap': None,
                'pe_ratio': None,
                'data_source': rt.get('source', 'live'),
            }})
        return jsonify({'success': False, 'error': f'Live price unavailable for {sym}.'})
    except Exception as e:
...
def api_live_data_status():
    ...
    # Data sourcing is an operational concern handled silently by the admin team.
    # NO user — regular or admin — should ever see a banner about brokers,
    # market hours, or data-source plumbing on the dashboard. Outages are
    # surfaced via Telegram to admin and on the Admin → Data API Plan page only.
    if not has_data_broker:
...
def sitemap_xml():
    ...
    urls = [
        ...
        ('https://www.targetcapital.ai/auth/login', '0.3', 'yearly'),
        ('https://www.targetcapital.ai/auth/register', '0.3', 'yearly'),
